...
def _dialog_novo_orcamento_UNUSED():
    ...
    intro_orc = st.text_area("📝 Introdução / Apresentação", key="_orc_intro",
                             height=80, placeholder="Texto introdutório da proposta (opcional)")
    obs_orc = st.text_area("📄 Observações", key="_orc_obs",
                           height=60, placeholder="Observações adicionais (opcional)")
    ...
    for _i in range(int(n_serv)):
        ...
        _svc_det = st.text_input(f"Detalhes do item {_i+1}", key=f"_orc_svc_det_{_i}",
        ...
        if _svc_nome:
            servicos_form.append({
                "servico": {
                    "id": _svc_id, "servico_id": _svc_id,
                    "nome_servico": _svc_nome, "detalhes": _svc_det,
                    "sigla_unidade": "H", "quantidade": str(_svc_qtd),
                    "valor_venda": str(_svc_preco), "tipo_desconto": "R$",
                    "desconto_valor": str(_svc_desc), "desconto_porcentagem": "0",
                }
            })
